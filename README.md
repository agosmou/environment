...
| Debian/Ubuntu servers | `platform/debian/packages` (apt; empty: a server's root layer is [fleet](https://github.com/agosmou/fleet)'s) | `platform/debian/bootstrap.sh`, which asserts `tailscaled` and installs nothing |
...
| `platform/debian/packages`, `platform/debian/bootstrap.sh` | The Debian/Ubuntu step for servers: asserts `tailscaled` is running and installs nothing; a server's root layer is the fleet repository's | `just sync` → bootstrap |
